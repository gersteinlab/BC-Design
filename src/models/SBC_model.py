...
class PointCloudMessagePassing(nn.Module):
    def __init__(self, feat_dim, edge_dim, l_max, num_scales, hidden_dim, aggregation='concat', num_heads=4):
        ...
        self.biochem_cls_token = nn.Parameter(torch.zeros(1, self.per_layer_dim))  # Adjusted dimension
    ...
    def forward(self, surfaces, biochem_feats):
        ...
        inf_row = torch.full((B, 1, N), float('inf'), device=surfaces.device)  # Last row with inf values
        zero_column = torch.zeros((B, N + 1, 1), device=surfaces.device)  # Last column with 0 values
        distances = torch.cat([distances, inf_row], dim=1)  # Add inf row
        distances = torch.cat([distances, zero_column], dim=2)  # Add 0 column
        N += 1  # Adjust N to N+1 since CLS token is added
...
class GraphTransformerModel(nn.Module):
    def __init__(self, hidden_dim, n_layers, n_heads, dropout=0.):
        ...
        self.struct_cls_token = nn.Parameter(torch.zeros(1, hidden_dim))
    ...
    def multi_head_attention(self, node_embeds, inv_distance_matrices, heat_kernel_pe, transformed_edge_weights, padding_mask):
        ...
        heat_kernel_pe = F.pad(heat_kernel_pe, (1, 0, 1, 0), "constant", 0)
        ...
        if padding_mask is not None:
            padding_mask = F.pad(padding_mask, (1, 0), "constant", 1)  # Pad to include CLS token
    ...
    def _pad_and_stack_edges(self, edge_weights, batch_id, E_idx, max_nodes):
        """Pad and stack edges to include interactions for CLS token."""
        B = batch_id.max().item() + 1  # Batch size
        ...
        for i in range(B):
            ...
            # Ensure CLS token interacts with all nodes (connect to all src_local and dst_local)
            padded_edges[i, 0, 1:len(node_indices) + 1] = 1  # CLS -> Nodes
            padded_edges[i, 1:len(node_indices) + 1, 0] = 1  # Nodes -> CLS

            padded_edges[i, src_local + 1, dst_local + 1] = edge_weights[local_edges_mask]
            padded_edges[i, dst_local + 1, src_local + 1] = edge_weights[local_edges_mask]  # Assuming undirected edges
...
class SBCModel(nn.Module):
    def __init__(self, args, queue_size=64, **kwargs):
        """ Graph labeling network """
        super(SBCModel, self).__init__()
        self.args = args
    ...
    def forward(self, batch):
        ...
        # Manually extract the CLS token from the structure encoder (first token)
        struct_cls_token = h_V_unflattened[:, 0, :]  # CLS token at position 0
        h_V_unflattened = h_V_unflattened[:, 1:, :]  # The rest of the node embeddings
        ...
        # Manually extract the CLS token from the biochemical encoder (first token)
        biochem_cls_token = h_surface[:, -1, :]  # CLS token at position -1
        h_surface = h_surface[:, :-1, :]  # The rest of the biochemical node embeddings
        ...
        contrastive_loss = self._contrastive_loss(struct_cls_token, biochem_cls_token)
        # contrastive_loss = 0

        # Update queues with current batch CLS tokens
        self._dequeue_and_enqueue(struct_cls_token, biochem_cls_token)
